...
from .core_algos_tree_dynamic_advantage import compute_grpo_tree_dynamic_advantage,AdvantageEstimator,agg_loss
...
class RayDAPOTrainer(RayPPOTrainer):
    def init_workers(self):
        ...
        if manager_class_fqn:
            ...
            from trae_verl.experimental.agent_loop import AgentLoopManager
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # add tree settings to batch
                batch.non_tensor_batch["branch_per_node"] = np.array(
                    [self.config.branch_per_node for _ in range(len(batch.batch))], dtype=object
                )
                batch.non_tensor_batch["max_depth"] = np.array(
                    [self.config.max_turns for _ in range(len(batch.batch))], dtype=object
                )
