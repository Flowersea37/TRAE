...
def get_reward_manager_cls(name: str) -> type[RewardManagerBase]:
    ...
    """
    print(f"[DEBUG] REWARD_LOOP_MANAGER_REGISTRY={REWARD_LOOP_MANAGER_REGISTRY}")
    if name not in REWARD_LOOP_MANAGER_REGISTRY:
        print(f"[DEBUG] REWARD_LOOP_MANAGER_REGISTRY={REWARD_LOOP_MANAGER_REGISTRY}")
        raise ValueError(f"Unknown reward manager: {name},[DEBUG] REWARD_LOOP_MANAGER_REGISTRY={REWARD_LOOP_MANAGER_REGISTRY}")
